...
class joint_distribution():
    ...
    def grid_prob(self, data, target1, target2, start):
        ...
        pdf_data = (count / (np.sum(2*data.shape[1])))#*((np.sum(data.shape[1]))/(data.shape[0]*data.shape[1]))
    ...
    def grid_loss(self):
        ...
        for i in range(1,self.Nx-1):
            for j in range(self.Ny-1):
                ...
                grid_prob = self.grid_prob(data, y[j], y[j + 1], i)
